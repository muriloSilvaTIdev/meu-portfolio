# Meu Portfólio Pessoal 💻

Olá! Sou o Murilo Almeida da Silva, estudante de Desenvolvimento de Software para Multiplataforma na Fatec Itaquera Prof. Miguel Reale. Este repositório contém o código-fonte do meu portfólio pessoal, onde apresento meus projetos e habilidades na área de tecnologia.

🔗 **[Clique aqui para ver o portfólio online](COLE_AQUI_O_LINK_DO_SEU_GITHUB_PAGES)**

## 🛠️ Tecnologias Utilizadas
* **Front-end:** HTML, CSS, JavaScript
* **Back-end & Banco de Dados:** Python, PHP, SQL, MySQL, Swift básico
* **Design:** Layout responsivo com tema escuro.

## 💼 Sobre a minha trajetória
Atualmente, busco uma oportunidade como Estagiário em Desenvolvimento de Software / TI para aplicar meus conhecimentos em Programação Web. Anteriormente, atuei como Jovem Aprendiz Administrativo no Hospital São Camilo SP, o que me trouxe forte capacidade de organização, responsabilidade e facilidade para trabalho em equipe. Complemento minha formação acadêmica com cursos extensivos, incluindo 400 horas em Programação Web.

## 🚀 Como executar este projeto localmente
1. Faça o clone deste repositório:
   `git clone https://github.com/muriloSilvaTIdev/portfolio.git`
2. Abra a pasta do projeto e execute o arquivo `index.html` no seu navegador.

---
📫 **Contato:**
* LinkedIn: [Seu Perfil do LinkedIn]
* Email: murilo102150@gmail.com
